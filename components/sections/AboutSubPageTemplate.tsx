...
import type { AboutContentBlock } from "@/lib/about/content";
...
export type AboutSubPageTemplateProps = {
  headline: string;
  subhead: string;
  heroImage: string;
...
export async function AboutSubPageTemplate({
  headline,
  subhead,
  heroImage,
  ...
  children,
  cta,
}: AboutSubPageTemplateProps) {
  const t = await getTranslations("about.subPage");
  ...
  return (
    <>
      ...
      {stats && stats.length > 0 ? <StatBar stats={stats} /> : null}
          ...
          <SectionContainer width="narrow">
            {intro.split(/\n\n+/).map((paragraph, index) => (
              <p
                ...
                className={cn("text-xl leading-relaxed text-graphite", index > 0 && "mt-6")}
              >
                {paragraph}
              </p>
            ))}
